"""Shared Firestore lookup tools — exception event and company profile.

Used by:
- ``modules/triage/agents/classifier`` (re-exported from its ``tools.py``)
- ``modules/triage/agents/impact`` (re-exported from its ``tools.py``)
- ``modules/triage/pipeline/hydration`` (deterministic pre-fetch callback)

Per ``.claude/rules/tools.md``: async for I/O, return
``{"status": "success"|"error", ...}``, per-turn cache in state. Both lookups
validate their Firestore documents with the canonical Pydantic models so the
returned ``data`` dict is well-shaped regardless of which caller consumes it.
"""
...
async def get_exception_event(
    ...
    Returns:
        ...
        ``data`` includes ``raw_content`` and ``metadata`` from the validated
        ``ExceptionEvent`` model.
    """
...
async def get_company_profile(
    ...
    Args:
        company_id: Firestore document ID for the company (3PL operator).
        tool_context: ADK tool context (provides state + actions).

    Returns:
        ``{"status": "success", "data": {...}, "markdown": str}`` on hit,
        ``{"status": "error", "error_message": str}`` on miss or failure.
        ``markdown`` is the ``## Business Context`` block ready for prompt
        injection.
    """
